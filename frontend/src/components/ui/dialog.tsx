...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 bg-[rgba(9,13,22,0.75)] backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
      className,
    )}
    {...props}
  />
...
>(({ className, children, showCloseButton = false, stackLevel = 'default', ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {showCloseButton && (
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md p-1.5 text-text-muted transition-colors hover:bg-[rgba(255,255,255,0.06)] hover:text-text-primary focus:outline-none">
          <X size={16} />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('text-[1.05rem] font-bold text-text-primary leading-none', className)}
    {...props}
  />
));
