  ...
  return (
    <div className="flex gap-2 sm:gap-3 mb-6 bg-[rgba(255,255,255,0.02)] p-1.5 rounded-xl border border-border-glass w-full lg:w-fit overflow-x-auto">
      ...
      <button
        ...
        className={`py-2 px-3 sm:px-4 rounded-lg text-[0.8rem] sm:text-[0.85rem] font-semibold flex items-center gap-2 transition-all duration-200 border-none outline-none whitespace-nowrap shrink-0 ${activeTab === 'notifications'
          ? 'bg-primary text-white shadow-[0_4px_12px_rgba(124,58,237,0.25)]'
          : 'text-text-secondary hover:text-text-primary bg-transparent'
          }`}
      >
        <Bell size={16} /> Manage Notifications
      </button>
    </div>
  );
}
