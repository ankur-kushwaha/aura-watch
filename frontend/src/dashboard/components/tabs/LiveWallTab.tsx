...
import { SlidersHorizontal, Video, RefreshCw, Server, Film, AlertTriangle } from 'lucide-react';
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 relative bg-gradient-to-br from-[#0c121e] to-[#060a12] w-full overflow-hidden select-none">
                    ...
                    {/* Hover action buttons for clips and alerts */}
                    {feed.streamId && (
                      <div
                        className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 z-20 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <button
                          onClick={() => navigate(`/app/events?streamId=${encodeURIComponent(feed.streamId || '')}`)}
                          className="py-1.5 text-nowrap px-3 text-[0.7rem] font-bold rounded-lg bg-[rgba(6,182,212,0.85)] hover:bg-[rgba(6,182,212,1)] text-white hover:text-white shadow-[0_4px_12px_rgba(6,182,212,0.3)] transition-all duration-200 flex items-center gap-1.5 cursor-pointer border-none outline-none"
                        >
                          <Film size={12} />
                          View Clips
                        </button>
                        <button
                          onClick={() => navigate(`/app/notifications?streamId=${encodeURIComponent(feed.streamId || '')}`)}
                          className="py-1.5 px-3  text-[0.7rem] font-bold rounded-lg bg-[rgba(239,68,68,0.85)] hover:bg-[rgba(239,68,68,1)] text-white hover:text-white shadow-[0_4px_12px_rgba(239,68,68,0.3)] transition-all duration-200 flex items-center gap-1.5 cursor-pointer border-none outline-none"
                        >
